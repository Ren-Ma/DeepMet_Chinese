...
if __name__ == '__main__':
    ...
    CCL_2018['sentence'] = CCL_2018.sentence.apply(lambda x: preprocssing(x)) \
                                 + "[SEP]" + CCL_2018.word.apply(lambda x: preprocssing(x)) \
                                 + "[SEP]" + CCL_2018.pos.apply(lambda x: preprocssing(x)) \
                                 + "[SEP]" + CCL_2018.tag.apply(lambda x: preprocssing(x))

    """['sentence2'] is the local feature consisting of the local sentence, word, pos, and tag. Ren Ma, 2021.12.02"""
    CCL_2018['sentence2'] = CCL_2018.local.apply(lambda x: preprocssing(x)) \
                                  + "[SEP]" + CCL_2018.word.apply(lambda x: preprocssing(x)) \
                                  + "[SEP]" + CCL_2018.pos.apply(lambda x: preprocssing(x)) \
                                  + "[SEP]" + CCL_2018.tag.apply(lambda x: preprocssing(x))
    ...
    model.load_weights(f'./model/model_onlyVerb-2.h5')  # load in trained model weights, model-2 is the best one
    ...
    CCL_2018 = CCL_2018.loc[:, ['id', 'tag', 'pos', 'predict', 'word', 'local', 'sentence']]
