...
def compute_input_arrays(df, columns, tokenizer, max_sequence_length):
    ...
    input_ids2, input_masks2, input_segments2 = [], [], []
    """df[columns] = ['id', 'sentence', 'word', 'label', 'pos', 'tag', 'local', 'sentence2']"""
...
if __name__ == '__main__':
    ...
    train = pd.read_csv('./RuiHe_Chinese_Metaphor_Detection-main/Data/PSUCMC_train_features.csv')
    # test = pd.read_csv('./data/VUA/VUA_verb_features.csv')
    # test2 = pd.read_csv('./data/VUA/VUA_allpos_features.csv')
    test = pd.read_csv('./RuiHe_Chinese_Metaphor_Detection-main/Data/PSUCMC_test_features.csv')
    test2 = pd.read_csv('./RuiHe_Chinese_Metaphor_Detection-main/Data/PSUCMC_test_features.csv')

    print('train shape =', train.shape)
    print('verb test shape =', test.shape)
    print('allpos test shape =', test2.shape)
    """['sentence'] is the global feature consisting of the whole sentence, word, pos, and tag. Ren Ma, 2021.12.02"""
    ...
    """['sentence2'] is the local feature consisting of the local sentence, word, pos, and tag. Ren Ma, 2021.12.02"""
    ...
    # np.save('./tempdata/train_inputs', inputs)
    ...
    # np.save('./tempdata/test_inputs', test_inputs)
    test_inputs = list(np.load('./tempdata/test_inputs.npy'))
    # test_inputs2 = compute_input_arrays(test2, input_categories, tokenizer, MAX_SEQUENCE_LENGTH)
    # np.save('./tempdata/test2_inputs', test_inputs2)
    test_inputs2 = list(np.load('./tempdata/test2_inputs.npy'))
    ...
    #     if os.path.isfile(f'./model/model-{fold}.h5'):
    #         model.load_weights(f'./model/model-{fold}.h5') # load in trained model weights
    #     else:
    ...
    #     pred += fold_pred  # sum up predictions over all folds,then take the mean.
    #     pred2 += fold_pred2
    ...
    # np.save('./tempdata/pred2.npy', pred2)
    pred = np.load('./tempdata/pred.npy')
    pred2 = np.load('./tempdata/pred2.npy')
